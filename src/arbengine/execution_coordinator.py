...
class ExecutionCoordinator:
    ...
    Independent bookmakers do not expose a distributed atomic commit. Sportage
    therefore serializes the difficult/manual primary first, uses strict automatic
    hedges, reconciles uncertain order state before retrying, and routes orphan
    exposure to a fresh rescue quote. Any unresolved state freezes new execution.
    """
    ...
    def _native_order(self, execution_id: str, index: int, leg, automatic: bool) -> BetOrder:
        ...
        tif = (
            ...
            if automatic and self.policy.require_full_fill_exchange
            else TimeInForce.DEFAULT
        )
        return BetOrder(
    ...
    def prepare(
        ...
        *,
        ...
        if opportunity.net_roi < self.policy.min_net_roi:
            raise ValueError(
                f"NET ROI {opportunity.net_roi:.3%} is below execution floor "
                f"{self.policy.min_net_roi:.3%}"
            )
        ...
        manual_count = sum(1 for automatic, _ in staged if not automatic)
        if manual_count > 1:
            raise ValueError(
                "Execution refuses plans with more than one manual primary leg; "
                "opening multiple manual exposures before an automatic hedge is not fail-closed"
            )
        if manual_count == 1 and not any(automatic for automatic, _ in staged):
            raise ValueError("Manual primary requires at least one supported automatic hedge leg")
        if self.policy.require_rescue_venue and not self._rescue_exists(opportunity, quotes):
            raise ValueError("No complete fresh automatic rescue route is available for this market")

        execution_id = f"exe_{uuid.uuid4().hex}"
        self.exec_store.acquire_lock(opportunity.event_market_key, execution_id)
        try:
            # Difficult/manual venue first; automatic exchange legs follow as hedges.
            staged.sort(key=lambda item: item[0])
            planned: list[PlannedLeg] = []
            has_manual = any(not automatic for automatic, _ in staged)
            for index, (automatic, leg) in enumerate(staged):
                role = LegRole.HEDGE if automatic and has_manual else LegRole.PRIMARY
            ...
            initial = RunStatus.WAITING_MANUAL if manual_count else RunStatus.PREPARED
            self.exec_store.create_run(
                execution_id,
                opportunity.event_market_key,
                initial.value,
                live,
    ...
    def confirm_manual_leg(
        ...
        *,
        ...
        if accepted and not plan.live:
            raise ValueError(
                "This execution plan was not armed for live use. Re-prepare with --live before placing "
                "a real manual primary."
            )
        ...
        if not accepted:
            result = ExecutionResult(
                ...
                message="Manual leg reported rejected by operator.",
                requested_stake=leg.order.stake,
        ...
        status = (
            ...
            if matched >= leg.order.stake
            else ExecutionStatus.PARTIALLY_MATCHED
        )
        result = ExecutionResult(
            operator_id=leg.operator_id,
            status=status,
            message="Manual placement confirmed.",
            bet_id=bet_id,
            customer_order_ref=leg.order.customer_order_ref,
            requested_stake=leg.order.stake,
            requested_odds=leg.order.limit_odds,
        ...
        self.exec_store.event(
            execution_id,
            "MANUAL_CONFIRMED",
            {"leg_id": leg_id, "matched": str(matched), "odds": str(odds)},
        )
        if status == ExecutionStatus.PARTIALLY_MATCHED:
            self.exec_store.update_run(
                execution_id,
                RunStatus.RESCUING.value,
                "Manual leg partially accepted",
            )
            return CoordinatorResult(
                execution_id=execution_id,
                status=RunStatus.RESCUING,
                message="Partial exposure requires rescue.",
            )

        self.exec_store.update_run(execution_id, RunStatus.PREPARED.value)
        return CoordinatorResult(
            execution_id=execution_id,
            status=RunStatus.PREPARED,
            message="Manual primary recorded; automatic hedge can proceed.",
        )
    ...
    def _safe_submit(self, leg: PlannedLeg, *, live: bool) -> ExecutionResult:
        ...
        if not preflight.ok:
            return ExecutionResult(
                operator_id=leg.operator_id,
                status=ExecutionStatus.REJECTED,
                message=f"Preflight failed: {preflight.message}",
                requested_stake=leg.order.stake,
                requested_odds=leg.order.limit_odds,
                customer_order_ref=leg.order.customer_order_ref,
            )
        ...
        try:
            ...
            # A transport timeout is not a rejection. Never submit again until the
            # persistent customerOrderRef has been reconciled.
            result = ExecutionResult(
                operator_id=leg.operator_id,
            ...
            for _ in range(self.policy.max_reconcile_attempts):
                reconciled = connector.reconcile_order(
                    customer_order_ref=order.customer_order_ref
                )
        ...
        elif result.status == ExecutionStatus.PARTIALLY_MATCHED and result.bet_id:
            connector.cancel_order(result.bet_id, market_id=order.market_id, live=live)
        return result
    ...
    def resume(
        ...
        *,
        ...
        if halted:
            return CoordinatorResult(
                execution_id=execution_id,
                status=RunStatus.EMERGENCY,
                message=f"Global halt: {reason}",
            )
        if row["status"] == RunStatus.WAITING_MANUAL.value:
            return CoordinatorResult(
                execution_id=execution_id,
                status=RunStatus.WAITING_MANUAL,
                message="Manual primary confirmation required.",
            )
        if row["status"] in {
            RunStatus.COMPLETED.value,
            RunStatus.RESCUED.value,
        ...
        if plan.live and not use_live:
            raise RuntimeError(
                "This plan is armed live; resume requires explicit live=True/--live. "
                "No hedge order was submitted."
            )
        if not plan.live and use_live:
            raise RuntimeError(
                "Dry-run plan cannot be upgraded to live at resume. Re-prepare with live=True/--live."
            )
        if not use_live:
            return CoordinatorResult(
                execution_id=execution_id,
                status=RunStatus.PREPARED,
                message="Dry-run plan only; no orders were submitted.",
            )

        self.exec_store.update_run(execution_id, RunStatus.EXECUTING.value)
        fresh_list = list(fresh_quotes)
        for leg in plan.legs:
            stored = next(
                r for r in self.exec_store.get_legs(execution_id) if r["leg_id"] == leg.leg_id
            )
            if stored["status"] == ExecutionStatus.ACCEPTED.value:
                continue
            if not leg.automatic:
                continue
            result = self._safe_submit(leg, live=True)
            self.exec_store.save_leg(
                execution_id,
                leg.leg_id,
                leg.role.value,
            ...
            self.exec_store.event(
                execution_id,
                "ORDER_RESULT",
                {"leg_id": leg.leg_id, "status": result.status.value},
            )
            if result.status == ExecutionStatus.UNKNOWN:
                return self._emergency(
                    plan,
                    "Order state UNKNOWN after reconciliation; duplicate/exposure risk",
                )
            if result.status != ExecutionStatus.ACCEPTED or not result.fully_matched:
                return self._rescue(
                    ...
                    fresh_list,
                    True,
                    f"Leg {leg.leg_id} not fully matched",
                )

        accepted = [
            r
            for r in self.exec_store.get_legs(execution_id)
            if r["status"] == ExecutionStatus.ACCEPTED.value
        ]
        if len(accepted) == len(plan.legs):
            ...
            return CoordinatorResult(
                ...
                message="All legs fully matched.",
            )
        return self._rescue(plan, fresh_list, True, "Execution ended with orphan exposure")
    ...
    def _accepted_exposure(self, plan: ExecutionPlan) -> tuple[dict[str, Decimal], Decimal]:
        ...
        for row in self.exec_store.get_legs(plan.execution_id):
            if row["leg_id"] not in leg_by_id or not row["result_json"]:
                continue
            ...
            outlay += (
                matched * (Decimal("1") + profile.stake_fee_pct)
                + profile.fixed_cost_per_bet
            )
    ...
    def _rescue(
        ...
        if not positive:
            self.exec_store.update_run(
                plan.execution_id,
                RunStatus.ABORTED.value,
                "No matched exposure after failed hedge",
            )
            self.exec_store.release_lock(plan.event_market_key, plan.execution_id)
            return CoordinatorResult(
                execution_id=plan.execution_id,
                status=RunStatus.ABORTED,
                message="No matched exposure remained.",
            )
        ...
        for outcome in plan.outcomes:
            ...
            for quote in quotes:
                if quote.event_id != plan.event_id:
                    continue
                if quote.market_signature != plan.market_signature:
                    continue
                ...
                slippage_bps = max(
                    Decimal("0"),
                    (reference - quote.odds) / reference * Decimal("10000"),
                )
            ...
            if not candidates:
                return self._emergency(
                    plan,
                    f"No rescue quote within policy for outcome {outcome}",
                )
            ...
            if quote.available_size is not None and quote.available_size < stake:
                return self._emergency(
                    plan,
                    f"Insufficient rescue depth for {outcome}: need {stake}, "
                    f"have {quote.available_size}",
                )
            cash = (
                stake * (Decimal("1") + profile.stake_fee_pct)
                + profile.fixed_cost_per_bet
            )
            rescue_outlay += cash
            order = BetOrder(
                ...
                customer_order_ref=(
                    f"rs-{plan.execution_id[-12:]}-{len(rescue_legs)}"[:32]
                ),
        ...
        if rescue_loss > self.policy.max_rescue_loss:
            return self._emergency(
                plan,
                f"Projected rescue loss {rescue_loss:.2f} exceeds limit "
                f"{self.policy.max_rescue_loss:.2f}",
            )
        if len(rescue_legs) > 1:
            return self._emergency(
                plan,
                "Multi-leg rescue requires atomic batch execution; intervention required",
            )
        ...
        for rescue in rescue_legs:
            ...
            self.exec_store.save_leg(
                ...
                result.status.value,
                rescue.order.model_dump(mode="json"),
                result.model_dump(mode="json"),
            )
            self.exec_store.event(
                plan.execution_id,
                "RESCUE_RESULT",
                {"leg_id": rescue.leg_id, "status": result.status.value},
            )
            if result.status != ExecutionStatus.ACCEPTED or not result.fully_matched:
                return self._emergency(
                    plan,
                    f"Rescue order {rescue.leg_id} failed to fully match",
                )

        self.exec_store.update_run(
            plan.execution_id,
            RunStatus.RESCUED.value,
            f"Rescued; projected floor {projected_profit:.2f}",
        )
        self.exec_store.event(
            plan.execution_id,
            "RESCUED",
            {
                ...
                "rescue_loss": str(rescue_loss),
            },
        )
        self.exec_store.release_lock(plan.event_market_key, plan.execution_id)
        return CoordinatorResult(
            execution_id=plan.execution_id,
            status=RunStatus.RESCUED,
            message="Orphan exposure hedged within policy.",
    ...
    def _emergency(self, plan: ExecutionPlan, reason: str) -> CoordinatorResult:
        ...
        # Keep the event lock: unresolved exposure must be consciously reconciled.
        return CoordinatorResult(
            execution_id=plan.execution_id,
            status=RunStatus.EMERGENCY,
            message=reason,
